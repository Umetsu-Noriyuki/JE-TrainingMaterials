...
`DISTINCT`は、**重複したデータを除外して、重複のない結果を取得する**ために使用します。
...
ここで`DISTINCT`を使用すると、重複している営業部を1つにまとめることができます。
...
`DISTINCT`は、後述する`JOIN`と組み合わせて、結合によって発生した重複を除外するときにも使用できます。
...
特に`EXISTS`は、**「条件に一致するデータが存在するか」を確認したい場合**に適しています。
...
ここでは、1人のユーザーに複数の注文がある場合に同じユーザーが複数回取得されるため、`DISTINCT`を使用して重複を除外しています。
...
[[ このページの先頭に戻る ]](#) [[ メニューに戻る ]](README.md#4-4)  
